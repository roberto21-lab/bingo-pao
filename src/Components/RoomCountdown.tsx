...
export default function RoomCountdown({ room }: RoomCountdownProps) {
  ...
  React.useEffect(() => {
    // CRÍTICO: Solo mostrar countdown si la sala es la siguiente en la cola (orderIndex === 1)
    // Las salas que no son la siguiente sala activa no deben mostrar contador
    if (room.orderIndex !== 1 && room.orderIndex !== null) {
  ...
  }, [room.scheduledAt, room.status, room.orderIndex]);
